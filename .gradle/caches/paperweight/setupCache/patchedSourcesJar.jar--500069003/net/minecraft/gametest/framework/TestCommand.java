...
public class TestCommand {
    ...
    private static int createNewStructure(CommandSourceStack source, String testName, int x, int y, int z) {
        if (x <= 48 && y <= 48 && z <= 48) {
            ...
            StructureUtils.createNewEmptyStructureBlock(testName.toLowerCase(), blockPos2, new Vec3i(x, y, z), Rotation.NONE, serverLevel);
    ...
    private static int exportTestStructure(CommandSourceStack source, String testName) {
        ...
        ResourceLocation resourceLocation = new ResourceLocation("minecraft", testName);
        Path path2 = source.getLevel().getStructureManager().getPathToGeneratedStructure(resourceLocation, ".nbt");
        Path path3 = NbtToSnbt.convertStructure(CachedOutput.NO_CACHE, path2, testName, path);
        if (path3 == null) {
    ...
    private static void say(ServerLevel world, String message, ChatFormatting formatting) {
        world.getPlayers((player) -> {
            ...
            player.sendSystemMessage(Component.literal(formatting + message));
        });
    }
