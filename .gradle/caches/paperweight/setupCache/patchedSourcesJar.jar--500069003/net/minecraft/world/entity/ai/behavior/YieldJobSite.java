...
public class YieldJobSite extends Behavior<Villager> {
    ...
    private boolean nearbyWantsJobsite(Holder<PoiType> poiType, Villager villager, BlockPos pos) {
        ...
        if (bl) {
            ...
            if (villagerProfession.heldJobSite().test(poiType)) {
                return !optional.isPresent() ? this.canReachPos(villager, pos, poiType.value()) : optional.get().pos().equals(pos);
            } else {
                return false;
            }
        }
    ...
    private boolean canReachPos(Villager villager, BlockPos pos, PoiType poiType) {
        Path path = villager.getNavigation().createPath(pos, poiType.validRange());
