            ...
            if (compoundTag != null && compoundTag.contains("BucketVariantTag", 3)) {
                ...
                for(int j = 0; j < TropicalFish.COMMON_VARIANTS.length; ++j) {
                    if (i == TropicalFish.COMMON_VARIANTS[j]) {
                        tooltip.add(Component.translatable(TropicalFish.getPredefinedName(j)).withStyle(chatFormattings));
                        return;
                    }
                }

                tooltip.add(Component.translatable(TropicalFish.getFishTypeName(i)).withStyle(chatFormattings));
