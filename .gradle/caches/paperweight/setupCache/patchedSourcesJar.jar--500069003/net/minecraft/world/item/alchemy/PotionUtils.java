    ...
    public static void addPotionTooltip(ItemStack stack, List<Component> list, float durationMultiplier) {
        ...
        if (list2.isEmpty()) {
            ...
            for(MobEffectInstance mobEffectInstance : list2) {
                ...
                if (mobEffectInstance.getDuration() > 20) {
                    mutableComponent = Component.translatable("potion.withDuration", mutableComponent, MobEffectUtil.formatDuration(mobEffectInstance, durationMultiplier));
                }
        ...
        if (!list3.isEmpty()) {
            ...
            for(Pair<Attribute, AttributeModifier> pair : list3) {
                ...
                if (d > 0.0D) {
                    list.add(Component.translatable("attribute.modifier.plus." + attributeModifier3.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(f), Component.translatable(pair.getFirst().getDescriptionId())).withStyle(ChatFormatting.BLUE));
                } else if (d < 0.0D) {
                    f *= -1.0D;
                    list.add(Component.translatable("attribute.modifier.take." + attributeModifier3.getOperation().toValue(), ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(f), Component.translatable(pair.getFirst().getDescriptionId())).withStyle(ChatFormatting.RED));
                }
            }
        }
