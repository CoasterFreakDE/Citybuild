...
public class EntityPositionSource implements PositionSource {
    ...
    private void resolveEntity(Level world) {
        this.entityOrUuidOrId.map(Optional::of, (either) -> {
            return Optional.ofNullable(either.map((uuid) -> {
                Entity var10000;
    ...
    private UUID getUuid() {
        return this.entityOrUuidOrId.map(Entity::getUUID, (either) -> {
            return either.map(Function.identity(), (integer) -> {
    ...
    int getId() {
        return this.entityOrUuidOrId.map(Entity::getId, (either) -> {
            return either.map((uUID) -> {
