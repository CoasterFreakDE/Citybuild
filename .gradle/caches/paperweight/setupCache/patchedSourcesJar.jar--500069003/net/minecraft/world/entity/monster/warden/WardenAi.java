...
public class WardenAi {
    ...
    private static final Behavior<Warden> DIG_COOLDOWN_SETTER = new Behavior<Warden>(ImmutableMap.of(MemoryModuleType.DIG_COOLDOWN, MemoryStatus.REGISTERED)) {
        ...
        protected void start(ServerLevel world, Warden entity, long time) {
            WardenAi.setDigCooldown(entity);
        }
    };
    ...
    private static void initFightActivity(Warden warden, Brain<Warden> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 10, ImmutableList.of(DIG_COOLDOWN_SETTER, new StopAttackingIfTargetInvalid<>((entity) -> {
            return !warden.getAngerLevel().isAngry() || !warden.canTargetEntity(entity);
        }, WardenAi::onTargetInvalid, false), new SetEntityLookTarget((entity) -> {
