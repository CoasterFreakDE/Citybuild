...
public class FireworkStarItem extends Item {
    ...
    public static void appendHoverText(CompoundTag nbt, List<Component> tooltip) {
        ...
        tooltip.add(Component.translatable("item.minecraft.firework_star.shape." + shape.getName()).withStyle(ChatFormatting.GRAY));
        int[] is = nbt.getIntArray("Colors");
        if (is.length > 0) {
            tooltip.add(appendColors(Component.empty().withStyle(ChatFormatting.GRAY), is));
        }

        int[] js = nbt.getIntArray("FadeColors");
        if (js.length > 0) {
            tooltip.add(appendColors(Component.translatable("item.minecraft.firework_star.fade_to").append(" ").withStyle(ChatFormatting.GRAY), js));
        }

        if (nbt.getBoolean("Trail")) {
            tooltip.add(Component.translatable("item.minecraft.firework_star.trail").withStyle(ChatFormatting.GRAY));
        }

        if (nbt.getBoolean("Flicker")) {
            tooltip.add(Component.translatable("item.minecraft.firework_star.flicker").withStyle(ChatFormatting.GRAY));
        }
