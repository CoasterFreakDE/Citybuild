...
public class IcebergFeature extends Feature<BlockStateConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<BlockStateConfiguration> context) {
        ...
        boolean bl3 = bl2 ? randomSource.nextDouble() > 0.1D : randomSource.nextDouble() > 0.7D;
        if (bl3) {
    ...
    private void generateCutOut(RandomSource randomSource, LevelAccessor world, int i, int j, BlockPos pos, boolean bl, int k, double d, int l) {
        ...
        double e = bl ? d + (Math.PI / 2D) : randomSource.nextDouble() * 2.0D * Math.PI;
        ...
        for(int s = -1; s > -j + randomSource.nextInt(5); --s) {
            int t = this.heightDependentRadiusSteep(randomSource, -s, j, i);
    ...
    private void generateIcebergBlock(LevelAccessor world, RandomSource randomSource, BlockPos pos, int height, int offsetX, int offsetY, int offsetZ, int i, int j, boolean bl, int k, double randomSine, boolean placeSnow, BlockState state) {
        double d = bl ? this.signedDistanceEllipse(offsetX, offsetZ, BlockPos.ZERO, j, this.getEllipseC(offsetY, height, k), randomSine) : this.signedDistanceCircle(offsetX, offsetZ, BlockPos.ZERO, i, randomSource);
        if (d < 0.0D) {
            BlockPos blockPos = pos.offset(offsetX, offsetY, offsetZ);
            double e = bl ? -0.5D : (double)(-6 - randomSource.nextInt(3));
            if (d > e && randomSource.nextDouble() > 0.9D) {
                return;
            }

            this.setIcebergBlock(blockPos, world, randomSource, height - offsetY, height, bl, placeSnow, state);
        }

    }

    private void setIcebergBlock(BlockPos pos, LevelAccessor world, RandomSource randomSource, int heightRemaining, int height, boolean lessSnow, boolean placeSnow, BlockState state) {
        ...
        if (blockState.getMaterial() == Material.AIR || blockState.is(Blocks.SNOW_BLOCK) || blockState.is(Blocks.ICE) || blockState.is(Blocks.WATER)) {
            boolean bl = !lessSnow || randomSource.nextDouble() > 0.05D;
            int i = lessSnow ? 3 : 2;
            if (placeSnow && !blockState.is(Blocks.WATER) && (double)heightRemaining <= (double)randomSource.nextInt(Math.max(1, height / i)) + (double)height * 0.6D && bl) {
    ...
    private double signedDistanceCircle(int x, int z, BlockPos pos, int i, RandomSource randomSource) {
        float f = 10.0F * Mth.clamp(randomSource.nextFloat(), 0.2F, 0.8F) / (float)i;
    ...
    private int heightDependentRadiusRound(RandomSource randomSource, int y, int height, int factor) {
        float f = 3.5F - randomSource.nextFloat();
        float g = (1.0F - (float)Math.pow((double)y, 2.0D) / ((float)height * f)) * (float)factor;
        if (height > 15 + randomSource.nextInt(5)) {
            int i = y < 3 + randomSource.nextInt(6) ? y / 2 : y;
    ...
    private int heightDependentRadiusSteep(RandomSource randomSource, int y, int height, int factor) {
