...
public class OreFeature extends Feature<OreConfiguration> {
    ...
    protected boolean doPlace(WorldGenLevel world, RandomSource randomSource, OreConfiguration config, double startX, double endX, double startZ, double endZ, double startY, double endY, int x, int y, int z, int horizontalSize, int verticalSize) {
        ...
        for(int k = 0; k < j; ++k) {
            ...
            double h = randomSource.nextDouble() * (double)j / 16.0D;
                                                ...
                                                if (world.ensureCanWrite(mutableBlockPos)) {
                                                    ...
                                                    if (levelChunkSection != null) {
                                                        ...
                                                        for(OreConfiguration.TargetBlockState targetBlockState : config.targetStates) {
                                                            if (canPlaceOre(blockState, bulkSectionAccess::getBlockState, randomSource, config, targetBlockState, mutableBlockPos)) {
    ...
    public static boolean canPlaceOre(BlockState state, Function<BlockPos, BlockState> posToState, RandomSource randomSource, OreConfiguration config, OreConfiguration.TargetBlockState target, BlockPos.MutableBlockPos pos) {
        if (!target.target.test(state, randomSource)) {
            return false;
        } else if (shouldSkipAirCheck(randomSource, config.discardChanceOnAirExposure)) {
            return true;
        } else {
    ...
    protected static boolean shouldSkipAirCheck(RandomSource randomSource, float chance) {
        if (chance <= 0.0F) {
            return true;
        } else if (chance >= 1.0F) {
            return false;
