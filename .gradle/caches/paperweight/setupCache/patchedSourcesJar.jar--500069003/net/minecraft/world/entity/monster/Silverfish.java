                    ...
                    for (int j = 0; j <= 10 && j >= -10; j = (j <= 0 ? 1 : 0) - j) {
                        for (int k = 0; k <= 10 && k >= -10; k = (k <= 0 ? 1 : 0) - k) {
                            ...
                            if (block instanceof InfestedBlock) {
                                // CraftBukkit start
                                if (org.bukkit.craftbukkit.v1_19_R1.event.CraftEventFactory.callEntityChangeBlockEvent(this.silverfish, blockposition1, net.minecraft.world.level.block.Blocks.AIR.defaultBlockState()).isCancelled()) {
                                    continue;
                                }
                                // CraftBukkit end
    ...
    private static class SilverfishMergeWithStoneGoal extends RandomStrollGoal {
        ...
        public void start() {
            if (!this.doMerge) {
                ...
                if (InfestedBlock.isCompatibleHostBlock(iblockdata)) {
                    // CraftBukkit start
                    if (org.bukkit.craftbukkit.v1_19_R1.event.CraftEventFactory.callEntityChangeBlockEvent(this.mob, blockposition, InfestedBlock.infestedStateByHost(iblockdata)).isCancelled()) {
                        return;
                    }
                    // CraftBukkit end
