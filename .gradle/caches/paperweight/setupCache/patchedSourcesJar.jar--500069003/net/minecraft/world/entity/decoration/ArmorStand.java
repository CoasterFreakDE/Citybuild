...
public class ArmorStand extends LivingEntity {
    ...
    public boolean hurt(DamageSource source, float amount) {
        if (!this.level.isClientSide && !this.isRemoved()) {
            if (DamageSource.OUT_OF_WORLD.equals(source)) {
                // CraftBukkit start
                if (org.bukkit.craftbukkit.v1_19_R1.event.CraftEventFactory.handleNonLivingEntityDamageEvent(this, source, amount)) {
                    return false;
                }
                // CraftBukkit end
                this.kill();
                ...
                // CraftBukkit start
                if (org.bukkit.craftbukkit.v1_19_R1.event.CraftEventFactory.handleNonLivingEntityDamageEvent(this, source, amount, true, this.invisible)) {
                    return false;
                }
                // CraftBukkit end
    ...
    private void brokenByPlayer(DamageSource damageSource) {
        drops.add(org.bukkit.craftbukkit.v1_19_R1.inventory.CraftItemStack.asBukkitCopy(new ItemStack(Items.ARMOR_STAND))); // CraftBukkit - add to drops
        this.brokenByAnything(damageSource);
    }
    ...
    private void brokenByAnything(DamageSource damageSource) {
        ...
        for (i = 0; i < this.handItems.size(); ++i) {
            ...
            if (!itemstack.isEmpty()) {
                drops.add(org.bukkit.craftbukkit.v1_19_R1.inventory.CraftItemStack.asCraftMirror(itemstack)); // CraftBukkit - add to drops // Paper - mirror so we can destroy it later - though this call site was safe
