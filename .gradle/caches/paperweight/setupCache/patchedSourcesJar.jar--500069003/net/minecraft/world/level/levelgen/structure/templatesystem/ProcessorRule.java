...
public class ProcessorRule {
    public static final Codec<ProcessorRule> CODEC = RecordCodecBuilder.create((instance) -> {
        return instance.group(RuleTest.CODEC.fieldOf("input_predicate").forGetter((rule) -> {
            return rule.inputPredicate;
        }), RuleTest.CODEC.fieldOf("location_predicate").forGetter((rule) -> {
            return rule.locPredicate;
        }), PosRuleTest.CODEC.optionalFieldOf("position_predicate", PosAlwaysTrueTest.INSTANCE).forGetter((rule) -> {
            return rule.posPredicate;
        }), BlockState.CODEC.fieldOf("output_state").forGetter((rule) -> {
            return rule.outputState;
        }), CompoundTag.CODEC.optionalFieldOf("output_nbt").forGetter((rule) -> {
            return Optional.ofNullable(rule.outputTag);
    ...
    public boolean test(BlockState input, BlockState location, BlockPos blockPos, BlockPos blockPos2, BlockPos pivot, RandomSource random) {
