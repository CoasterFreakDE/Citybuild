...
public class FeatureSorter {
    public static <T> List<FeatureSorter.StepFeatureData> buildFeaturesPerStep(List<T> biomes, Function<T, List<HolderSet<PlacedFeature>>> biomesToPlacedFeaturesList, boolean listInvolvedBiomesOnFailure) {
        ...
        for(T object : biomes) {
            ...
            for(int j = 0; j < list2.size(); ++j) {
                for(Holder<PlacedFeature> holder : list2.get(j)) {
                    ...
                    list.add(new FeatureData(object2IntMap.computeIfAbsent(placedFeature, (feature) -> {
            ...
            for(int k = 0; k < list.size(); ++k) {
                Set<FeatureData> set = map.computeIfAbsent(list.get(k), (feature) -> {
        ...
        for(int m = 0; m < i; ++m) {
            int n = m;
            List<PlacedFeature> list5 = list3.stream().filter((feature) -> {
                return feature.step() == n;
    ...
    public static record StepFeatureData(List<PlacedFeature> features, ToIntFunction<PlacedFeature> indexMapping) {
        StepFeatureData(List<PlacedFeature> features) {
            this(features, Util.createIndexLookup(features, (size) -> {
                return new Object2IntOpenCustomHashMap<>(size, Util.identityStrategy());
            }));
        }
    }
}
