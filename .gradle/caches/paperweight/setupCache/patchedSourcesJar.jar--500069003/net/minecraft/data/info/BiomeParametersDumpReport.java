...
public class BiomeParametersDumpReport implements DataProvider {
    ...
    public void run(CachedOutput writer) {
        ...
        MultiNoiseBiomeSource.Preset.getPresets().forEach((pair) -> {
            MultiNoiseBiomeSource multiNoiseBiomeSource = pair.getSecond().biomeSource(registry, false);
            dumpValue(this.createPath(pair.getFirst()), writer, dynamicOps, MultiNoiseBiomeSource.CODEC, multiNoiseBiomeSource);
        });
    }
    ...
    private static <E> void dumpValue(Path path, CachedOutput cachedOutput, DynamicOps<JsonElement> dynamicOps, Encoder<E> encoder, E object) {
        try {
            Optional<JsonElement> optional = encoder.encodeStart(dynamicOps, object).resultOrPartial((string) -> {
