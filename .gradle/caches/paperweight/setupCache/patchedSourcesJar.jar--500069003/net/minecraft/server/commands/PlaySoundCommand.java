        ...
        while(true) {
            ...
            while(true) {
                if (!var13.hasNext()) {
                    ...
                    if (targets.size() == 1) {
                        source.sendSuccess(Component.translatable("commands.playsound.success.single", sound, targets.iterator().next().getDisplayName()), true);
                    } else {
                        source.sendSuccess(Component.translatable("commands.playsound.success.multiple", sound, targets.size()), true);
                    }

                    return i;
                }

                serverPlayer = (ServerPlayer)var13.next();
            ...
            serverPlayer.connection.send(new ClientboundCustomSoundPacket(sound, category, vec3, j, pitch, l));
            ++i;
        }
    }
}
