...
public class RecipeCommand {
    ...
    private static int giveRecipes(CommandSourceStack source, Collection<ServerPlayer> targets, Collection<Recipe<?>> recipes) throws CommandSyntaxException {
        ...
        if (i == 0) {
            ...
            if (targets.size() == 1) {
                source.sendSuccess(Component.translatable("commands.recipe.give.success.single", recipes.size(), targets.iterator().next().getDisplayName()), true);
            } else {
    ...
    private static int takeRecipes(CommandSourceStack source, Collection<ServerPlayer> targets, Collection<Recipe<?>> recipes) throws CommandSyntaxException {
        ...
        if (i == 0) {
            ...
            if (targets.size() == 1) {
                source.sendSuccess(Component.translatable("commands.recipe.take.success.single", recipes.size(), targets.iterator().next().getDisplayName()), true);
            } else {
