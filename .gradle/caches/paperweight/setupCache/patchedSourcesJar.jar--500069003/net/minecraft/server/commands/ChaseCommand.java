...
public class ChaseCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("chase").then(Commands.literal("follow").then(Commands.argument("host", StringArgumentType.string()).executes((context) -> {
            return follow(context.getSource(), StringArgumentType.getString(context, "host"), 10000);
        }).then(Commands.argument("port", IntegerArgumentType.integer(1, 65535)).executes((context) -> {
            return follow(context.getSource(), StringArgumentType.getString(context, "host"), IntegerArgumentType.getInteger(context, "port"));
        }))).executes((context) -> {
            return follow(context.getSource(), "localhost", 10000);
        })).then(Commands.literal("lead").then(Commands.argument("bind_address", StringArgumentType.string()).executes((context) -> {
            return lead(context.getSource(), StringArgumentType.getString(context, "bind_address"), 10000);
        }).then(Commands.argument("port", IntegerArgumentType.integer(1024, 65535)).executes((context) -> {
            return lead(context.getSource(), StringArgumentType.getString(context, "bind_address"), IntegerArgumentType.getInteger(context, "port"));
        }))).executes((context) -> {
            return lead(context.getSource(), "0.0.0.0", 10000);
    ...
    private static int follow(CommandSourceStack source, String ip, int port) {
        if (alreadyRunning(source)) {
            ...
            chaseClient.start();
            source.sendSuccess(Component.literal("You are now chasing " + ip + ":" + port + ". If that server does '/chase lead' then you will automatically go to the same position. Use '/chase stop' to stop chasing."), false);
            return 0;
        }
    }
}
