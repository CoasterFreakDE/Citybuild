...
public class ScoreboardCommand {
    ...
    private static final Dynamic2CommandExceptionType ERROR_NO_VALUE = new Dynamic2CommandExceptionType((objective, target) -> {
        return Component.translatable("commands.scoreboard.players.get.null", objective, target);
    });
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("scoreboard").requires((source) -> {
            ...
            return addObjective(context.getSource(), StringArgumentType.getString(context, "objective"), ObjectiveCriteriaArgument.getCriteria(context, "criteria"), Component.literal(StringArgumentType.getString(context, "objective")));
    ...
    private static int getScore(CommandSourceStack source, String target, Objective objective) throws CommandSyntaxException {
        ...
        if (!scoreboard.hasPlayerScore(target, objective)) {
            ...
            source.sendSuccess(Component.translatable("commands.scoreboard.players.get.success", target, score.getScore(), objective.getFormattedDisplayName()), false);
    ...
    private static int performOperation(CommandSourceStack source, Collection<String> targets, Objective targetObjective, OperationArgument.Operation operation, Collection<String> sources, Objective sourceObjectives) throws CommandSyntaxException {
        ...
        if (targets.size() == 1) {
            source.sendSuccess(Component.translatable("commands.scoreboard.players.operation.success.single", targetObjective.getFormattedDisplayName(), targets.iterator().next(), i), true);
        } else {
    ...
    private static int enableTrigger(CommandSourceStack source, Collection<String> targets, Objective objective) throws CommandSyntaxException {
        if (objective.getCriteria() != ObjectiveCriteria.TRIGGER) {
            ...
            if (i == 0) {
                ...
                if (targets.size() == 1) {
                    source.sendSuccess(Component.translatable("commands.scoreboard.players.enable.success.single", objective.getFormattedDisplayName(), targets.iterator().next()), true);
                } else {
    ...
    private static int resetScores(CommandSourceStack source, Collection<String> targets) {
        ...
        if (targets.size() == 1) {
            source.sendSuccess(Component.translatable("commands.scoreboard.players.reset.all.single", targets.iterator().next()), true);
        } else {
    ...
    private static int resetScore(CommandSourceStack source, Collection<String> targets, Objective objective) {
        ...
        if (targets.size() == 1) {
            source.sendSuccess(Component.translatable("commands.scoreboard.players.reset.specific.single", objective.getFormattedDisplayName(), targets.iterator().next()), true);
        } else {
    ...
    private static int setScore(CommandSourceStack source, Collection<String> targets, Objective objective, int score) {
        ...
        if (targets.size() == 1) {
            source.sendSuccess(Component.translatable("commands.scoreboard.players.set.success.single", objective.getFormattedDisplayName(), targets.iterator().next(), score), true);
        } else {
    ...
    private static int addScore(CommandSourceStack source, Collection<String> targets, Objective objective, int score) {
        ...
        if (targets.size() == 1) {
            source.sendSuccess(Component.translatable("commands.scoreboard.players.add.success.single", score, objective.getFormattedDisplayName(), targets.iterator().next(), i), true);
        } else {
    ...
    private static int removeScore(CommandSourceStack source, Collection<String> targets, Objective objective, int score) {
        ...
        if (targets.size() == 1) {
            source.sendSuccess(Component.translatable("commands.scoreboard.players.remove.success.single", score, objective.getFormattedDisplayName(), targets.iterator().next(), i), true);
        } else {
    ...
    private static int listTrackedPlayers(CommandSourceStack source) {
        ...
        if (collection.isEmpty()) {
            ...
            source.sendSuccess(Component.translatable("commands.scoreboard.players.list.success", collection.size(), ComponentUtils.formatList(collection)), false);
        }
    ...
    private static int listTrackedPlayerScores(CommandSourceStack source, String target) {
        ...
        if (map.isEmpty()) {
            source.sendSuccess(Component.translatable("commands.scoreboard.players.list.entity.empty", target), false);
        } else {
            source.sendSuccess(Component.translatable("commands.scoreboard.players.list.entity.success", target, map.size()), false);

            for(Map.Entry<Objective, Score> entry : map.entrySet()) {
                source.sendSuccess(Component.translatable("commands.scoreboard.players.list.entity.entry", entry.getKey().getFormattedDisplayName(), entry.getValue().getScore()), false);
            }
        }
    ...
    private static int clearDisplaySlot(CommandSourceStack source, int slot) throws CommandSyntaxException {
        ...
        if (scoreboard.getDisplayObjective(slot) == null) {
            ...
            source.sendSuccess(Component.translatable("commands.scoreboard.objectives.display.cleared", Scoreboard.getDisplaySlotNames()[slot]), true);
            return 0;
        }
    }
    ...
    private static int setDisplaySlot(CommandSourceStack source, int slot, Objective objective) throws CommandSyntaxException {
        ...
        if (scoreboard.getDisplayObjective(slot) == objective) {
            ...
            source.sendSuccess(Component.translatable("commands.scoreboard.objectives.display.set", Scoreboard.getDisplaySlotNames()[slot], objective.getDisplayName()), true);
            return 0;
        }
    }
