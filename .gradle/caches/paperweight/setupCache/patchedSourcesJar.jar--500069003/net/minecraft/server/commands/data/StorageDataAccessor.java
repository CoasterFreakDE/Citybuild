...
public class StorageDataAccessor implements DataAccessor {
    ...
    public Component getPrintSuccess(Tag element) {
        return Component.translatable("commands.data.storage.query", this.id, NbtUtils.toPrettyComponent(element));
    }

    @Override
    public Component getPrintSuccess(NbtPathArgument.NbtPath path, double scale, int result) {
        return Component.translatable("commands.data.storage.get", path, this.id, String.format(Locale.ROOT, "%.2f", scale), result);
    }
}
