...
public class AdvancementCommands {
    ...
    private static int perform(CommandSourceStack source, Collection<ServerPlayer> targets, AdvancementCommands.Action operation, Collection<Advancement> selection) {
        ...
        if (i == 0) {
            ...
            if (selection.size() == 1) {
                if (targets.size() == 1) {
                    source.sendSuccess(Component.translatable(operation.getKey() + ".one.to.one.success", selection.iterator().next().getChatComponent(), targets.iterator().next().getDisplayName()), true);
                } else {
                    source.sendSuccess(Component.translatable(operation.getKey() + ".one.to.many.success", selection.iterator().next().getChatComponent(), targets.size()), true);
                }
            } else if (targets.size() == 1) {
                source.sendSuccess(Component.translatable(operation.getKey() + ".many.to.one.success", selection.size(), targets.iterator().next().getDisplayName()), true);
            } else {
    ...
    private static int performCriterion(CommandSourceStack source, Collection<ServerPlayer> targets, AdvancementCommands.Action operation, Advancement advancement, String criterion) {
        ...
        if (!advancement.getCriteria().containsKey(criterion)) {
            ...
            if (i == 0) {
                ...
                if (targets.size() == 1) {
                    source.sendSuccess(Component.translatable(operation.getKey() + ".criterion.to.one.success", criterion, advancement.getChatComponent(), targets.iterator().next().getDisplayName()), true);
                } else {
                    source.sendSuccess(Component.translatable(operation.getKey() + ".criterion.to.many.success", criterion, advancement.getChatComponent(), targets.size()), true);
                }
