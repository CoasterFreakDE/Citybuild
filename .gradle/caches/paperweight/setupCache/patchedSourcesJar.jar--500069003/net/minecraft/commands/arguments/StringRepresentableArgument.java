...
public class StringRepresentableArgument<T extends Enum<T> & StringRepresentable> implements ArgumentType<T> {
    private static final DynamicCommandExceptionType ERROR_INVALID_VALUE = new DynamicCommandExceptionType((value) -> {
        return Component.translatable("argument.enum.invalid", value);
    });
    ...
    public T parse(StringReader stringReader) throws CommandSyntaxException {
        ...
        return this.codec.parse(JsonOps.INSTANCE, new JsonPrimitive(string)).result().orElseThrow(() -> {
            return ERROR_INVALID_VALUE.create(string);
        });
    }

    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> commandContext, SuggestionsBuilder suggestionsBuilder) {
        return SharedSuggestionProvider.suggest(Arrays.<Enum>stream((Enum[])this.values.get()).map((enum_) -> {
            return ((StringRepresentable)enum_).getSerializedName();
        }).collect(Collectors.toList()), suggestionsBuilder);
    }

    public Collection<String> getExamples() {
        return Arrays.<Enum>stream((Enum[])this.values.get()).map((enum_) -> {
            return ((StringRepresentable)enum_).getSerializedName();
