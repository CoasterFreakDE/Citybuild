...
public class LighthingBoltPredicate implements EntitySubPredicate {
    ...
    public boolean matches(Entity entity, ServerLevel world, @Nullable Vec3 pos) {
        if (!(entity instanceof LightningBolt lightningBolt)) {
            ...
            return this.blocksSetOnFire.matches(lightningBolt.getBlocksSetOnFire()) && (this.entityStruck == EntityPredicate.ANY || lightningBolt.getHitEntities().anyMatch((entityx) -> {
                return this.entityStruck.matches(world, pos, entityx);
            }));
        }
    }
}
