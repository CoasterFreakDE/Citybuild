...
public record PlacedFeature(Holder<ConfiguredFeature<?, ?>> feature, List<PlacementModifier> placement) {
    ...
    public boolean place(WorldGenLevel world, ChunkGenerator generator, RandomSource randomSource, BlockPos pos) {
        return this.placeWithContext(new PlacementContext(world, generator, Optional.empty()), randomSource, pos);
    }

    public boolean placeWithBiomeCheck(WorldGenLevel world, ChunkGenerator generator, RandomSource randomSource, BlockPos pos) {
        return this.placeWithContext(new PlacementContext(world, generator, Optional.of(this)), randomSource, pos);
    }

    private boolean placeWithContext(PlacementContext context, RandomSource randomSource, BlockPos pos) {
        Stream<BlockPos> stream = Stream.of(pos);

        for(PlacementModifier placementModifier : this.placement) {
            stream = stream.flatMap((posx) -> {
                return placementModifier.getPositions(context, randomSource, posx);
            });
        }
        ...
        stream.forEach((blockPos) -> {
            if (configuredFeature.place(context.getLevel(), context.generator(), randomSource, blockPos)) {
