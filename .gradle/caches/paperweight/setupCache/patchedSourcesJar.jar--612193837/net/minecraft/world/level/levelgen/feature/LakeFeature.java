...
public class LakeFeature extends Feature<LakeFeature.Configuration> {
    ...
    public boolean place(FeaturePlaceContext<LakeFeature.Configuration> context) {
        ...
        if (blockPos.getY() <= worldGenLevel.getMinBuildHeight() + 4) {
            ...
            for(int j = 0; j < i; ++j) {
                double d = randomSource.nextDouble() * 6.0D + 3.0D;
                double e = randomSource.nextDouble() * 4.0D + 2.0D;
                double f = randomSource.nextDouble() * 6.0D + 3.0D;
                double g = randomSource.nextDouble() * (16.0D - d - 2.0D) + 1.0D + d / 2.0D;
                double h = randomSource.nextDouble() * (8.0D - e - 4.0D) + 2.0D + e / 2.0D;
                double k = randomSource.nextDouble() * (16.0D - f - 2.0D) + 1.0D + f / 2.0D;
