    ...
    protected void start(ServerLevel world, LivingEntity entity, long time) {
        ...
        if (iblockdata.is(BlockTags.WOODEN_DOORS, (blockbase_blockdata) -> {
            ...
            if (!blockdoor.isOpen(iblockdata)) {
                ...
                org.bukkit.event.entity.EntityInteractEvent event = new org.bukkit.event.entity.EntityInteractEvent(entity.getBukkitEntity(), org.bukkit.craftbukkit.v1_19_R1.block.CraftBlock.at(entity.level, blockposition));
        ...
        if (iblockdata1.is(BlockTags.WOODEN_DOORS, (blockbase_blockdata) -> {
            ...
            if (!blockdoor1.isOpen(iblockdata1)) {
                // CraftBukkit start - entities opening doors
                org.bukkit.event.entity.EntityInteractEvent event = new org.bukkit.event.entity.EntityInteractEvent(entity.getBukkitEntity(), org.bukkit.craftbukkit.v1_19_R1.block.CraftBlock.at(entity.level, blockposition1));
                entity.level.getCraftServer().getPluginManager().callEvent(event);
                if (event.isCancelled()) {
                    return;
                }
