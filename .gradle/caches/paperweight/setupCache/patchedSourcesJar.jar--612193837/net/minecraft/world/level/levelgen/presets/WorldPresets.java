...
public class WorldPresets {
    ...
    private static ResourceKey<WorldPreset> register(String id) {
        return ResourceKey.create(Registry.WORLD_PRESET_REGISTRY, new ResourceLocation(id));
    }

    public static Optional<ResourceKey<WorldPreset>> fromSettings(WorldGenSettings generatorOptions) {
        ChunkGenerator chunkGenerator = generatorOptions.overworld();
    ...
    public static WorldGenSettings createNormalWorldFromPreset(RegistryAccess dynamicRegistryManager, long seed, boolean generateStructures, boolean bonusChest) {
        return dynamicRegistryManager.registryOrThrow(Registry.WORLD_PRESET_REGISTRY).getHolderOrThrow(NORMAL).value().createWorldGenSettings(seed, generateStructures, bonusChest);
    }

    public static WorldGenSettings createNormalWorldFromPreset(RegistryAccess dynamicRegistryManager, long seed) {
        return createNormalWorldFromPreset(dynamicRegistryManager, seed, true, false);
    }

    public static WorldGenSettings createNormalWorldFromPreset(RegistryAccess dynamicRegistryManager) {
        return createNormalWorldFromPreset(dynamicRegistryManager, RandomSource.create().nextLong());
    }

    public static WorldGenSettings demoSettings(RegistryAccess dynamicRegistryManager) {
        return createNormalWorldFromPreset(dynamicRegistryManager, (long)"North Carolina".hashCode(), true, true);
    }

    public static LevelStem getNormalOverworld(RegistryAccess dynamicRegistryManager) {
        return dynamicRegistryManager.registryOrThrow(Registry.WORLD_PRESET_REGISTRY).getHolderOrThrow(NORMAL).value().overworldOrThrow();
    }
    ...
    static class Bootstrap {
        ...
        private LevelStem makeNoiseBasedOverworld(BiomeSource biomeSource, Holder<NoiseGeneratorSettings> chunkGeneratorSettings) {
            return this.makeOverworld(new NoiseBasedChunkGenerator(this.structureSets, this.noises, biomeSource, chunkGeneratorSettings));
        }
        ...
        private Holder<WorldPreset> registerCustomOverworldPreset(ResourceKey<WorldPreset> key, LevelStem dimensionOptions) {
            return BuiltinRegistries.register(this.presets, key, this.createPresetWithCustomOverworld(dimensionOptions));
        }
