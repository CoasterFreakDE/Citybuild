...
public final class BelowZeroRetrogen {
    ...
    private static final Codec<BitSet> BITSET_CODEC = Codec.LONG_STREAM.xmap((serializedBedrockBitSet) -> {
        return BitSet.valueOf(serializedBedrockBitSet.toArray());
    }, (bedrockBitSet) -> {
        return LongStream.of(bedrockBitSet.toLongArray());
    });
    private static final Codec<ChunkStatus> NON_EMPTY_CHUNK_STATUS = Registry.CHUNK_STATUS.byNameCodec().comapFlatMap((status) -> {
        return status == ChunkStatus.EMPTY ? DataResult.error("target_status cannot be empty") : DataResult.success(status);
    }, Function.identity());
