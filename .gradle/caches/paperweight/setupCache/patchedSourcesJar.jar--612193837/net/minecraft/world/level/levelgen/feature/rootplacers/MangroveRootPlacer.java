...
public class MangroveRootPlacer extends RootPlacer {
    ...
    public static final Codec<MangroveRootPlacer> CODEC = RecordCodecBuilder.create((instance) -> {
        return rootPlacerParts(instance).and(MangroveRootPlacement.CODEC.fieldOf("mangrove_root_placement").forGetter((rootPlacer) -> {
    ...
    private boolean simulateRoots(LevelSimulatedReader world, RandomSource random, BlockPos pos, Direction direction, BlockPos origin, List<BlockPos> offshootPositions, int rootLength) {
    ...
    protected boolean canPlaceRoot(LevelSimulatedReader world, BlockPos pos) {
        return super.canPlaceRoot(world, pos) || world.isStateAtPosition(pos, (state) -> {
            return state.is(this.mangroveRootPlacement.canGrowThrough());
        });
    }
    ...
    protected void placeRoot(LevelSimulatedReader world, BiConsumer<BlockPos, BlockState> replacer, RandomSource random, BlockPos pos, TreeConfiguration config) {
        if (world.isStateAtPosition(pos, (state) -> {
            return state.is(this.mangroveRootPlacement.muddyRootsIn());
        })) {
