...
public class FancyTrunkPlacer extends TrunkPlacer {
    ...
    private boolean makeLimb(LevelSimulatedReader levelSimulatedReader, BiConsumer<BlockPos, BlockState> biConsumer, RandomSource randomSource, BlockPos startPos, BlockPos branchPos, boolean make, TreeConfiguration config) {
        if (!make && Objects.equals(startPos, branchPos)) {
            ...
            for(int j = 0; j <= i; ++j) {
                ...
                if (make) {
                    this.placeLog(levelSimulatedReader, biConsumer, randomSource, blockPos2, config, (state) -> {
                        return state.setValue(RotatedPillarBlock.AXIS, this.getLogAxis(startPos, blockPos2));
                    });
                } else if (!this.isFree(levelSimulatedReader, blockPos2)) {
                    return false;
                }
            }
    ...
    private void makeBranches(LevelSimulatedReader world, BiConsumer<BlockPos, BlockState> replacer, RandomSource randomSource, int treeHeight, BlockPos startPos, List<FancyTrunkPlacer.FoliageCoords> branchPositions, TreeConfiguration config) {
        for(FancyTrunkPlacer.FoliageCoords foliageCoords : branchPositions) {
            ...
            if (!blockPos.equals(foliageCoords.attachment.pos()) && this.trimBranches(treeHeight, i - startPos.getY())) {
                this.makeLimb(world, replacer, randomSource, blockPos, foliageCoords.attachment.pos(), true, config);
            }
        }
