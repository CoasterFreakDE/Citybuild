...
public class PoiCompetitorScan extends Behavior<Villager> {
    ...
    protected void start(ServerLevel world, Villager entity, long time) {
        ...
        world.getPoiManager().getType(globalPos.pos()).ifPresent((holder) -> {
            BehaviorUtils.getNearbyVillagersWithCondition(entity, (villager) -> {
                return this.competesForSameJobsite(globalPos, holder, villager);
    ...
    private boolean competesForSameJobsite(GlobalPos pos, Holder<PoiType> poiType, Villager villager) {
    ...
    private boolean hasMatchingProfession(Holder<PoiType> poiType, VillagerProfession profession) {
