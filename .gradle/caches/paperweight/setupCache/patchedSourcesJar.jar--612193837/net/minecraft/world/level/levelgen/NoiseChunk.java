...
public class NoiseChunk implements DensityFunction.ContextProvider, DensityFunction.FunctionContext {
    ...
    public NoiseChunk(int horizontalCellCount, RandomState noiseConfig, int startX, int startZ, NoiseSettings generationShapeConfig, DensityFunctions.BeardifierOrMarker beardifying, NoiseGeneratorSettings chunkGeneratorSettings, Aquifer.FluidPicker fluidLevelSampler, Blender blender) {
        ...
        this.cellCountY = Mth.intFloorDiv(generationShapeConfig.height(), this.cellHeight);
        this.cellNoiseMinY = Mth.intFloorDiv(generationShapeConfig.minY(), this.cellHeight);
        this.firstCellX = Math.floorDiv(startX, this.cellWidth);
        this.firstCellZ = Math.floorDiv(startZ, this.cellWidth);
        ...
        if (!chunkGeneratorSettings.isAquifersEnabled()) {
            ...
            int o = SectionPos.blockToSectionCoord(startX);
            int p = SectionPos.blockToSectionCoord(startZ);
    ...
    private void fillSlice(boolean start, int nextCellX) {
        this.cellStartBlockX = nextCellX * this.cellWidth;
        this.inCellX = 0;
    ...
    public void updateForY(int blockY, double cellDeltaY) {
    ...
    public void updateForX(int blockX, double cellDeltaX) {
        this.inCellX = blockX - this.cellStartBlockX;
        this.interpolators.forEach((interpolator) -> {
            interpolator.updateForX(cellDeltaX);
        });
    }

    public void updateForZ(int blockZ, double cellDeltaZ) {
        this.inCellZ = blockZ - this.cellStartBlockZ;
        ++this.interpolationCounter;
        this.interpolators.forEach((interpolator) -> {
            interpolator.updateForZ(cellDeltaZ);
        });
    }
