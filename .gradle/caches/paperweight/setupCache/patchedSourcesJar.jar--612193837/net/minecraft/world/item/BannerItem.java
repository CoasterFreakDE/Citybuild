            ...
            for(int i = 0; i < listTag.size() && i < 6; ++i) {
                ...
                if (holder != null) {
                    holder.unwrapKey().map((key) -> {
                        return key.location().toShortLanguageKey();
                    }).ifPresent((translationKey) -> {
                        tooltip.add(Component.translatable("block.minecraft.banner." + translationKey + "." + dyeColor.getName()).withStyle(ChatFormatting.GRAY));
                    });
                }
            }
