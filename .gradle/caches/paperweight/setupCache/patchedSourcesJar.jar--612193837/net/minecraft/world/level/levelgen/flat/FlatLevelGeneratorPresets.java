...
public class FlatLevelGeneratorPresets {
    ...
    private static ResourceKey<FlatLevelGeneratorPreset> register(String id) {
        return ResourceKey.create(Registry.FLAT_LEVEL_GENERATOR_PRESET_REGISTRY, new ResourceLocation(id));
    }
    ...
    static class Bootstrap {
        ...
        private Holder<FlatLevelGeneratorPreset> register(ResourceKey<FlatLevelGeneratorPreset> registryKey, ItemLike icon, ResourceKey<Biome> biome, Set<ResourceKey<StructureSet>> structureSets, boolean hasFeatures, boolean hasLakes, FlatLayerInfo... layers) {
            HolderSet.Direct<StructureSet> direct = HolderSet.direct(structureSets.stream().flatMap((structureSet) -> {
                return this.structureSets.getHolder(structureSet).stream();
            ...
            flatLevelGeneratorSettings.setBiome(this.biomes.getOrCreateHolderOrThrow(biome));
            return BuiltinRegistries.register(this.presets, registryKey, new FlatLevelGeneratorPreset(icon.asItem().builtInRegistryHolder(), flatLevelGeneratorSettings));
        }
