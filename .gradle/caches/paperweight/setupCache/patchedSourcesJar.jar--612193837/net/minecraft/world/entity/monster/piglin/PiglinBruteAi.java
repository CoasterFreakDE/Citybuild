...
public class PiglinBruteAi {
    ...
    private static void initFightActivity(PiglinBrute piglinBrute, Brain<PiglinBrute> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 10, ImmutableList.of(new StopAttackingIfTargetInvalid<>((entity) -> {
            return !isNearestValidAttackTarget(piglinBrute, entity);
    ...
    private static boolean isNearestValidAttackTarget(AbstractPiglin piglin, LivingEntity entity) {
        return findNearestValidAttackTarget(piglin).filter((target) -> {
    ...
    private static Optional<? extends LivingEntity> getTargetIfWithinRange(AbstractPiglin piglin, MemoryModuleType<? extends LivingEntity> targetMemoryModule) {
        return piglin.getBrain().getMemory(targetMemoryModule).filter((target) -> {
            return target.closerThan(piglin, 12.0D);
        });
    }
