...
public class LargeDripstoneFeature extends Feature<LargeDripstoneConfiguration> {
        ...
        if (!DripstoneUtils.isEmptyOrWater(worldGenLevel, blockPos)) {
            ...
            if (optional.isPresent() && optional.get() instanceof Column.Range) {
                ...
                if (range.height() < 4) {
                    ...
                    LargeDripstoneFeature.LargeDripstone largeDripstone = makeDripstone(blockPos.atY(range.ceiling() - 1), false, randomSource, k, largeDripstoneConfiguration.stalactiteBluntness, largeDripstoneConfiguration.heightScale);
                    LargeDripstoneFeature.LargeDripstone largeDripstone2 = makeDripstone(blockPos.atY(range.floor() + 1), true, randomSource, k, largeDripstoneConfiguration.stalagmiteBluntness, largeDripstoneConfiguration.heightScale);
    ...
    private static LargeDripstoneFeature.LargeDripstone makeDripstone(BlockPos pos, boolean isStalagmite, RandomSource randomSource, int scale, FloatProvider bluntness, FloatProvider heightScale) {
        return new LargeDripstoneFeature.LargeDripstone(pos, isStalagmite, scale, (double)bluntness.sample(randomSource), (double)heightScale.sample(randomSource));
    }
    ...
    static final class LargeDripstone {
        ...
        void placeBlocks(WorldGenLevel world, RandomSource randomSource, LargeDripstoneFeature.WindOffsetter wind) {
    ...
    static final class WindOffsetter {
        ...
        WindOffsetter(int y, RandomSource randomSource, FloatProvider wind) {
            this.originY = y;
            float f = wind.sample(randomSource);
