...
public class WardenEntitySensor extends NearestLivingEntitySensor<Warden> {
    ...
    protected void doTick(ServerLevel world, Warden entity) {
        ...
        getClosest(entity, (entityx) -> {
            return entityx.getType() == EntityType.PLAYER;
        }).or(() -> {
            return getClosest(entity, (livingEntity) -> {
                return livingEntity.getType() != EntityType.PLAYER;
            });
        }).ifPresentOrElse((entityx) -> {
            entity.getBrain().setMemory(MemoryModuleType.NEAREST_ATTACKABLE, entityx);
        }, () -> {
    ...
    private static Optional<LivingEntity> getClosest(Warden warden, Predicate<LivingEntity> targetPredicate) {
        return warden.getBrain().getMemory(MemoryModuleType.NEAREST_LIVING_ENTITIES).stream().flatMap(Collection::stream).filter(warden::canTargetEntity).filter(targetPredicate).findFirst();
    }
