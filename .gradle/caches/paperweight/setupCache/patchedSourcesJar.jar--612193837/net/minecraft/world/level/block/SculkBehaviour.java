...
public interface SculkBehaviour {
    SculkBehaviour DEFAULT = new SculkBehaviour() {
        ...
        public boolean attemptSpreadVein(LevelAccessor world, BlockPos pos, BlockState state, @Nullable Collection<Direction> directions, boolean markForPostProcessing) {
            if (directions == null) {
                ...
                return !state.isAir() && !state.getFluidState().is(Fluids.WATER) ? false : SculkVeinBlock.regrow(world, pos, state, directions);
            } else {
                return SculkBehaviour.super.attemptSpreadVein(world, pos, state, directions, markForPostProcessing);
            }
        }
    ...
    default boolean attemptSpreadVein(LevelAccessor world, BlockPos pos, BlockState state, @Nullable Collection<Direction> directions, boolean markForPostProcessing) {
