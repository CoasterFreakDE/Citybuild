...
public abstract class AbstractPoiSectionFix extends DataFix {
    ...
    public AbstractPoiSectionFix(Schema schema, String name) {
        super(schema, false);
        this.name = name;
    }
    ...
    private <T> Dynamic<T> processSectionRecords(Dynamic<T> dynamic) {
        return DataFixUtils.orElse(dynamic.asStreamOpt().result().map((dynamics) -> {
            return dynamic.createList(this.processRecords(dynamics));
        }), dynamic);
    }
