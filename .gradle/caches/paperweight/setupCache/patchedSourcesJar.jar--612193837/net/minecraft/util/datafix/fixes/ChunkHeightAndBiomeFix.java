...
public class ChunkHeightAndBiomeFix extends DataFix {
        ...
        return this.fixTypeEverywhereTyped("ChunkHeightAndBiomeFix", type, type2, (chunk) -> {
            return chunk.updateTyped(opticFinder, type3, (level) -> {
                ...
                level = level.updateTyped(opticFinder2, type4, (sections) -> {
                    ...
                    Dynamic<?> dynamic3 = sections.write().result().orElseThrow(() -> {
    ...
    private static Dynamic<?>[] getBiomeContainers(Dynamic<?> level, boolean overworld, int i, MutableBoolean heightAlreadyUpdated) {
        ...
        int[] is = level.get("Biomes").asIntStreamOpt().result().map(IntStream::toArray).orElse((int[])null);
        if (is != null && is.length == 1536) {
            heightAlreadyUpdated.setValue(true);

            for(int j = 0; j < 24; ++j) {
                dynamics[j] = makeBiomeContainer(level, (sectionY) -> {
                    return getOldBiome(is, j * 64 + sectionY);
                });
            }
        } else if (is != null && is.length == 1024) {
            for(int l = 0; l < 16; ++l) {
                int m = l - i;
                dynamics[m] = makeBiomeContainer(level, (sectionY) -> {
                    return getOldBiome(is, l * 64 + sectionY);
                });
            }

            if (overworld) {
                Dynamic<?> dynamic = makeBiomeContainer(level, (sectionY) -> {
                    return getOldBiome(is, sectionY % 16);
                });
                Dynamic<?> dynamic2 = makeBiomeContainer(level, (sectionY) -> {
                    return getOldBiome(is, sectionY % 16 + 1008);
                });
    ...
    private static Dynamic<?> updateChunkTag(Dynamic<?> level, boolean overworld, boolean heightAlreadyUpdated, boolean atNoiseStatus, Supplier<ChunkProtoTickListFix.PoorMansPalettedContainer> supplier) {
        ...
        if (!overworld) {
            ...
            level = addPaddingEntries(level, "Lights");
            ...
            if (!atNoiseStatus) {
                ...
                if (optional.isPresent()) {
                    ...
                    if (!"empty".equals(string)) {
                        level = level.set("blending_data", level.createMap(ImmutableMap.of(level.createString("old_noise"), level.createBoolean(STATUS_IS_OR_AFTER_NOISE.contains(string)))));
    ...
    private static Dynamic<?> makeBiomeContainer(Dynamic<?> level, Int2IntFunction biomeGetter) {
        ...
        Dynamic<?> dynamic = level.createList(int2IntMap.keySet().stream().map((rawBiomeId) -> {
            return level.createString(BIOMES_BY_ID.getOrDefault(rawBiomeId.intValue(), "minecraft:plains"));
        }));
