...
public class FunctionArgument implements ArgumentType<FunctionArgument.Result> {
    ...
    private static final DynamicCommandExceptionType ERROR_UNKNOWN_TAG = new DynamicCommandExceptionType((id) -> {
        return Component.translatable("arguments.function.tag.unknown", id);
    });
    private static final DynamicCommandExceptionType ERROR_UNKNOWN_FUNCTION = new DynamicCommandExceptionType((id) -> {
        return Component.translatable("arguments.function.unknown", id);
    });
    ...
    public FunctionArgument.Result parse(StringReader stringReader) throws CommandSyntaxException {
        if (stringReader.canRead() && stringReader.peek() == '#') {
            ...
            return new FunctionArgument.Result() {
                ...
                @Override
                public Pair<ResourceLocation, Either<CommandFunction, Collection<CommandFunction>>> unwrap(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
    ...
    static Collection<CommandFunction> getFunctionTag(CommandContext<CommandSourceStack> context, ResourceLocation id) throws CommandSyntaxException {
        Collection<CommandFunction> collection = context.getSource().getServer().getFunctions().getTag(id);
        if (collection == null) {
    ...
    public static Pair<ResourceLocation, Either<CommandFunction, Collection<CommandFunction>>> getFunctionOrTag(CommandContext<CommandSourceStack> context, String name) throws CommandSyntaxException {
    ...
    public interface Result {
        ...
        Pair<ResourceLocation, Either<CommandFunction, Collection<CommandFunction>>> unwrap(CommandContext<CommandSourceStack> context) throws CommandSyntaxException;
    }
}
