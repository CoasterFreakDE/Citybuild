...
public class TextComponentTagVisitor implements TagVisitor {
    ...
    public void visitByte(ByteTag element) {
        ...
        this.result = Component.literal(String.valueOf((Object)element.getAsNumber())).append(component).withStyle(SYNTAX_HIGHLIGHTING_NUMBER);
    }
    ...
    public void visitInt(IntTag element) {
        this.result = Component.literal(String.valueOf((Object)element.getAsNumber())).withStyle(SYNTAX_HIGHLIGHTING_NUMBER);
    }
    ...
    public void visitCompound(CompoundTag compound) {
        if (compound.isEmpty()) {
            ...
            for(Iterator<String> iterator = collection.iterator(); iterator.hasNext(); mutableComponent.append(mutableComponent2)) {
                ...
                mutableComponent2 = Component.literal(Strings.repeat(this.indentation, this.depth + 1)).append(handleEscapePretty(string)).append(NAME_VALUE_SEPARATOR).append(" ").append((new TextComponentTagVisitor(this.indentation, this.depth + 1)).visit(compound.get(string)));
                if (iterator.hasNext()) {
