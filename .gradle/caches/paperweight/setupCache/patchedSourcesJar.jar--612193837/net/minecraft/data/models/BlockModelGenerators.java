...
public class BlockModelGenerators {
    ...
    private static BlockStateGenerator createNorthWestMirroredCubeGenerator(Block block, ResourceLocation modelId, TextureMapping textures, BiConsumer<ResourceLocation, Supplier<JsonElement>> modelCollector) {
    ...
    private void createActiveRail(Block rail) {
        ...
        PropertyDispatch propertyDispatch = PropertyDispatch.properties(BlockStateProperties.POWERED, BlockStateProperties.RAIL_SHAPE_STRAIGHT).generate((on, shape) -> {
            switch (shape) {
                case NORTH_SOUTH:
                    return Variant.variant().with(VariantProperties.MODEL, on ? resourceLocation4 : resourceLocation);
                case EAST_WEST:
                    return Variant.variant().with(VariantProperties.MODEL, on ? resourceLocation4 : resourceLocation).with(VariantProperties.Y_ROT, VariantProperties.Rotation.R90);
                case ASCENDING_EAST:
                    return Variant.variant().with(VariantProperties.MODEL, on ? resourceLocation5 : resourceLocation2).with(VariantProperties.Y_ROT, VariantProperties.Rotation.R90);
                case ASCENDING_WEST:
                    return Variant.variant().with(VariantProperties.MODEL, on ? resourceLocation6 : resourceLocation3).with(VariantProperties.Y_ROT, VariantProperties.Rotation.R90);
    ...
    private void createSculkShrieker() {
        ...
        this.blockStateOutput.accept(MultiVariantGenerator.multiVariant(Blocks.SCULK_SHRIEKER).with(createBooleanModelDispatch(BlockStateProperties.CAN_SUMMON, resourceLocation2, resourceLocation)));
    }
    ...
    private void createRepeater() {
        ...
        this.blockStateOutput.accept(MultiVariantGenerator.multiVariant(Blocks.REPEATER).with(PropertyDispatch.properties(BlockStateProperties.DELAY, BlockStateProperties.LOCKED, BlockStateProperties.POWERED).generate((tick, locked, on) -> {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append('_').append((Object)tick).append("tick");
            if (on) {
    ...
    private void createSnowBlocks() {
        ...
        this.blockStateOutput.accept(MultiVariantGenerator.multiVariant(Blocks.SNOW).with(PropertyDispatch.property(BlockStateProperties.LAYERS).generate((height) -> {
            return Variant.variant().with(VariantProperties.MODEL, height < 8 ? ModelLocationUtils.getModelLocation(Blocks.SNOW, "_height" + height * 2) : resourceLocation);
        })));
    ...
    private void createStructureBlock() {
        ...
        this.blockStateOutput.accept(MultiVariantGenerator.multiVariant(Blocks.STRUCTURE_BLOCK).with(PropertyDispatch.property(BlockStateProperties.STRUCTUREBLOCK_MODE).generate((mode) -> {
            return Variant.variant().with(VariantProperties.MODEL, this.createSuffixedVariant(Blocks.STRUCTURE_BLOCK, "_" + mode.getSerializedName(), ModelTemplates.CUBE_ALL, TextureMapping::cube));
        })));
    }

    private void createSweetBerryBush() {
        this.createSimpleFlatItemModel(Items.SWEET_BERRIES);
        this.blockStateOutput.accept(MultiVariantGenerator.multiVariant(Blocks.SWEET_BERRY_BUSH).with(PropertyDispatch.property(BlockStateProperties.AGE_3).generate((stage) -> {
            return Variant.variant().with(VariantProperties.MODEL, this.createSuffixedVariant(Blocks.SWEET_BERRY_BUSH, "_stage" + stage, ModelTemplates.CROSS, TextureMapping::cross));
        })));
    }
    ...
    private void createMultiface(Block block) {
        ...
        Condition.TerminalCondition terminalCondition = Util.make(Condition.condition(), (terminalConditionx) -> {
            MULTIFACE_GENERATOR.stream().map(Pair::getFirst).forEach((property) -> {
    ...
    private void createSculkCatalyst() {
        ...
        this.blockStateOutput.accept(MultiVariantGenerator.multiVariant(Blocks.SCULK_CATALYST).with(PropertyDispatch.property(BlockStateProperties.BLOOM).generate((boolean_) -> {
            return Variant.variant().with(VariantProperties.MODEL, boolean_ ? resourceLocation3 : resourceLocation2);
        })));
