...
public abstract class PropertyDispatch {
    ...
    private void verifyComplete() {
        ...
        for(Property<?> property : list) {
            stream = stream.flatMap((propertiesMap) -> {
                return property.getAllValues().map(propertiesMap::extend);
            });
        }

        List<Selector> list2 = stream.filter((propertiesMap) -> {
            return !this.values.containsKey(propertiesMap);
