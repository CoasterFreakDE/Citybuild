...
public class TerrainProvider {
    ...
    private static final ToFloatFunction<Float> AMPLIFIED_OFFSET = ToFloatFunction.createUnlimited((value) -> {
        return value < 0.0F ? value : value * 2.0F;
    });
    private static final ToFloatFunction<Float> AMPLIFIED_FACTOR = ToFloatFunction.createUnlimited((value) -> {
        return 1.25F - 6.25F / (value + 5.0F);
    });
    private static final ToFloatFunction<Float> AMPLIFIED_JAGGEDNESS = ToFloatFunction.createUnlimited((value) -> {
        return value * 2.0F;
    });
    ...
    public static <C, I extends ToFloatFunction<C>> CubicSpline<C, I> overworldJaggedness(I continents, I erosion, I ridges, I ridgesFolded, boolean amplified) {
        ...
        float f = 0.65F;
        return CubicSpline.<C, I>builder(continents, toFloatFunction).addPoint(-0.11F, 0.0F).addPoint(0.03F, buildErosionJaggednessSpline(erosion, ridges, ridgesFolded, 1.0F, 0.5F, 0.0F, 0.0F, toFloatFunction)).addPoint(0.65F, buildErosionJaggednessSpline(erosion, ridges, ridgesFolded, 1.0F, 1.0F, 1.0F, 0.0F, toFloatFunction)).build();
    }
    ...
    private static <C, I extends ToFloatFunction<C>> CubicSpline<C, I> buildErosionJaggednessSpline(I erosion, I ridges, I ridgesFolded, float f, float g, float h, float i, ToFloatFunction<Float> amplifier) {
        ...
        return CubicSpline.<C, I>builder(erosion, amplifier).addPoint(-1.0F, cubicSpline).addPoint(-0.78F, cubicSpline2).addPoint(-0.5775F, cubicSpline2).addPoint(-0.375F, 0.0F).build();
    }
    ...
    private static <C, I extends ToFloatFunction<C>> CubicSpline<C, I> getErosionFactor(I erosion, I ridges, I ridgesFolded, float f, boolean bl, ToFloatFunction<Float> amplifier) {
        ...
        CubicSpline.Builder<C, I> builder = CubicSpline.<C, I>builder(erosion, amplifier).addPoint(-0.6F, cubicSpline).addPoint(-0.5F, CubicSpline.<C, I>builder(ridges, amplifier).addPoint(-0.05F, 6.3F).addPoint(0.05F, 2.67F).build()).addPoint(-0.35F, cubicSpline).addPoint(-0.25F, cubicSpline).addPoint(-0.1F, CubicSpline.<C, I>builder(ridges, amplifier).addPoint(-0.05F, 2.67F).addPoint(0.05F, 6.3F).build()).addPoint(0.03F, cubicSpline);
        if (bl) {
    ...
    public static <C, I extends ToFloatFunction<C>> CubicSpline<C, I> buildErosionOffsetSpline(I erosion, I ridgesFolded, float continentalness, float f, float g, float h, float i, float j, boolean bl, boolean bl2, ToFloatFunction<Float> amplifier) {
        ...
        CubicSpline<C, I> cubicSpline4 = ridgeSpline(ridgesFolded, continentalness - 0.15F, 0.5F * h, Mth.lerp(0.5F, 0.5F, 0.5F) * h, 0.5F * h, 0.6F * h, 0.5F, amplifier);
        ...
        CubicSpline<C, I> cubicSpline8 = CubicSpline.<C, I>builder(ridgesFolded, amplifier).addPoint(-1.0F, continentalness).addPoint(-0.4F, cubicSpline6).addPoint(0.0F, g + 0.07F).build();
        CubicSpline<C, I> cubicSpline9 = ridgeSpline(ridgesFolded, -0.02F, j, j, f, g, 0.0F, amplifier);
        CubicSpline.Builder<C, I> builder = CubicSpline.<C, I>builder(erosion, amplifier).addPoint(-0.85F, cubicSpline).addPoint(-0.7F, cubicSpline2).addPoint(-0.4F, cubicSpline3).addPoint(-0.35F, cubicSpline4).addPoint(-0.1F, cubicSpline5).addPoint(0.2F, cubicSpline6);
        if (bl) {
    ...
    private static <C, I extends ToFloatFunction<C>> CubicSpline<C, I> ridgeSpline(I ridgesFolded, float continentalness, float f, float g, float h, float i, float j, ToFloatFunction<Float> amplifier) {
        ...
        float l = 5.0F * (g - f);
        return CubicSpline.<C, I>builder(ridgesFolded, amplifier).addPoint(-1.0F, continentalness, k).addPoint(-0.4F, f, Math.min(k, l)).addPoint(0.0F, g, l).addPoint(0.4F, h, 2.0F * (h - g)).addPoint(1.0F, i, 0.7F * (i - h)).build();
    }
}
