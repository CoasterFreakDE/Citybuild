...
public class WorldgenRegistryDumpReport implements DataProvider {
    ...
    private <T> void dumpRegistryCap(CachedOutput writer, RegistryAccess registryManager, DynamicOps<JsonElement> ops, RegistryAccess.RegistryData<T> registry) {
        ResourceKey<? extends Registry<T>> resourceKey = registry.key();
        Registry<T> registry2 = registryManager.ownedRegistryOrThrow(resourceKey);
        ...
        for(Map.Entry<ResourceKey<T>, T> entry : registry2.entrySet()) {
            dumpValue(pathProvider.json(entry.getKey().location()), writer, ops, registry.codec(), entry.getValue());
        }
