...
public class DeserializationContext {
    ...
    public final LootItemCondition[] deserializeConditions(JsonArray array, String key, LootContextParamSet contextType) {
        ...
        ValidationContext validationContext = new ValidationContext(contextType, this.predicateManager::get, (tableId) -> {
            return null;
        });
        ...
        for(LootItemCondition lootItemCondition : lootItemConditions) {
            ...
            validationContext.getProblems().forEach((name, message) -> {
                LOGGER.warn("Found validation problem in advancement trigger {}/{}: {}", key, name, message);
            });
        }
