...
public class ClientboundUpdateAdvancementsPacket implements Packet<ClientGamePacketListener> {
    ...
    public void write(FriendlyByteBuf buf) {
        ...
        buf.writeMap(this.added, FriendlyByteBuf::writeResourceLocation, (buf2, task) -> {
            task.serializeToNetwork(buf2);
        });
        buf.writeCollection(this.removed, FriendlyByteBuf::writeResourceLocation);
        buf.writeMap(this.progress, FriendlyByteBuf::writeResourceLocation, (buf2, progress) -> {
            progress.serializeToNetwork(buf2);
        });
    }
