...
public class ClientboundCommandsPacket implements Packet<ClientGamePacketListener> {
    ...
    public void write(FriendlyByteBuf buf) {
        buf.writeCollection(this.entries, (buf2, node) -> {
            node.write(buf2);
        });
    ...
    private static void validateEntries(List<ClientboundCommandsPacket.Entry> nodeDatas, BiPredicate<ClientboundCommandsPacket.Entry, IntSet> validator) {
        ...
        while(!intSet.isEmpty()) {
            boolean bl = intSet.removeIf((i) -> {
                return validator.test(nodeDatas.get(i), intSet);
            });
            if (!bl) {
    ...
    private static ClientboundCommandsPacket.Entry createEntry(CommandNode<SharedSuggestionProvider> node, Object2IntMap<CommandNode<SharedSuggestionProvider>> nodes) {
        int i = 0;
        int j;
    ...
    public RootCommandNode<SharedSuggestionProvider> getRoot(CommandBuildContext commandRegistryAccess) {
        return (RootCommandNode)(new ClientboundCommandsPacket.NodeResolver(commandRegistryAccess, this.entries)).resolve(this.rootIndex);
    }
