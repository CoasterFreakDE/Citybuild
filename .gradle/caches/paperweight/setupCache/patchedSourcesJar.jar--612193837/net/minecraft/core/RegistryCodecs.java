...
public class RegistryCodecs {
    private static <T> MapCodec<RegistryCodecs.RegistryEntry<T>> withNameAndId(ResourceKey<? extends Registry<T>> registryRef, MapCodec<T> elementCodec) {
        return RecordCodecBuilder.mapCodec((instance) -> {
            return instance.group(ResourceKey.codec(registryRef).fieldOf("name").forGetter(RegistryCodecs.RegistryEntry::key), Codec.INT.fieldOf("id").forGetter(RegistryCodecs.RegistryEntry::id), elementCodec.forGetter(RegistryCodecs.RegistryEntry::value)).apply(instance, RegistryCodecs.RegistryEntry::new);
        });
    }
    ...
    private static <E> Decoder<Registry<E>> dataPackAwareDecoder(final ResourceKey<? extends Registry<E>> registryRef, final Codec<E> codec, Decoder<Map<ResourceKey<E>, E>> entryMapDecoder, Lifecycle lifecycle) {
    ...
    private static <T> Codec<Map<ResourceKey<T>, T>> directCodec(ResourceKey<? extends Registry<T>> registryRef, Codec<T> elementCodec) {
        return Codec.unboundedMap(ResourceKey.codec(registryRef), elementCodec);
    }
