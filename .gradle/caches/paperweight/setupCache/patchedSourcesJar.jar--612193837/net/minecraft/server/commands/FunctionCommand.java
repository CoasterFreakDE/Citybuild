...
public class FunctionCommand {
    ...
    private static int runFunction(CommandSourceStack source, Collection<CommandFunction> functions) {
        ...
        if (functions.size() == 1) {
            source.sendSuccess(Component.translatable("commands.function.success.single", i, functions.iterator().next().getId()), true);
        } else {
            source.sendSuccess(Component.translatable("commands.function.success.multiple", i, functions.size()), true);
        }
