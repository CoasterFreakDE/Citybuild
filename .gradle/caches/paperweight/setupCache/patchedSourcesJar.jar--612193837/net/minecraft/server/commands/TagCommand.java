    ...
    private static int listTags(CommandSourceStack source, Collection<? extends Entity> targets) {
        ...
        if (targets.size() == 1) {
            ...
            if (set.isEmpty()) {
                ...
                source.sendSuccess(Component.translatable("commands.tag.list.single.success", entity2.getDisplayName(), set.size(), ComponentUtils.formatList(set)), false);
            }
        } else if (set.isEmpty()) {
            source.sendSuccess(Component.translatable("commands.tag.list.multiple.empty", targets.size()), false);
        } else {
            source.sendSuccess(Component.translatable("commands.tag.list.multiple.success", targets.size(), set.size(), ComponentUtils.formatList(set)), false);
        }
