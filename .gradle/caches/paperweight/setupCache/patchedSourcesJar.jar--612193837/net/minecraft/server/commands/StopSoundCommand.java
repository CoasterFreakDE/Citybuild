...
public class StopSoundCommand {
    ...
    private static int stopSound(CommandSourceStack source, Collection<ServerPlayer> targets, @Nullable SoundSource category, @Nullable ResourceLocation sound) {
        ...
        if (category != null) {
            if (sound != null) {
                source.sendSuccess(Component.translatable("commands.stopsound.success.source.sound", sound, category.getName()), true);
            } else {
                source.sendSuccess(Component.translatable("commands.stopsound.success.source.any", category.getName()), true);
            }
        } else if (sound != null) {
            source.sendSuccess(Component.translatable("commands.stopsound.success.sourceless.sound", sound), true);
        } else {
