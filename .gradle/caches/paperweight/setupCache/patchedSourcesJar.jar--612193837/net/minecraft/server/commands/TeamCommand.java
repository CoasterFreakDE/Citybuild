...
public class TeamCommand {
    ...
    private static int leaveTeam(CommandSourceStack source, Collection<String> members) {
        ...
        if (members.size() == 1) {
            source.sendSuccess(Component.translatable("commands.team.leave.success.single", members.iterator().next()), true);
        } else {
    ...
    private static int joinTeam(CommandSourceStack source, PlayerTeam team, Collection<String> members) {
        ...
        if (members.size() == 1) {
            source.sendSuccess(Component.translatable("commands.team.join.success.single", members.iterator().next(), team.getFormattedDisplayName()), true);
        } else {
    ...
    private static int setCollision(CommandSourceStack source, PlayerTeam team, Team.CollisionRule collisionRule) throws CommandSyntaxException {
        if (team.getCollisionRule() == collisionRule) {
            ...
            source.sendSuccess(Component.translatable("commands.team.option.collisionRule.success", team.getFormattedDisplayName(), collisionRule.getDisplayName()), true);
            return 0;
        }
    }
    ...
    private static int listMembers(CommandSourceStack source, PlayerTeam team) {
        ...
        if (collection.isEmpty()) {
            ...
            source.sendSuccess(Component.translatable("commands.team.list.members.success", team.getFormattedDisplayName(), collection.size(), ComponentUtils.formatList(collection)), false);
        }
    ...
    private static int listTeams(CommandSourceStack source) {
        ...
        if (collection.isEmpty()) {
            ...
            source.sendSuccess(Component.translatable("commands.team.list.teams.success", collection.size(), ComponentUtils.formatList(collection, PlayerTeam::getFormattedDisplayName)), false);
        }
